...
- (void)viewDidLoad {
    ...
    if (self.fileType == APKFileTypeVideo) {
        ...
        typeKey = @"事件";
    }else if (self.fileType == APKFileTypeSecurity)
        typeKey = @"泊车";
    ...
    [[APKDVRCommandFactory setCommandWithProperty:@"Playback" value:@"enter"] execute:^(id responseObject) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [APKDVR sharedInstance].isJoinPlayBackMode = YES;
...
- (IBAction)clickedButtomDownloadBtn:(UIButton *)sender {
    
    [self clickDownloadButton:sender];
}
...
- (IBAction)clickedButtomDeleteBtn:(UIButton *)sender {
...
//    [self clickSelectButton:self.selectButton];
    ...
    NSString *message = [NSString stringWithFormat:NSLocalizedString(@"删除%d个文件？", nil)];
...
- (IBAction)chooseBtnClicked:(UIButton *)sender {
    ...
    self.tableView.editing = sender.isSelected ? NO : YES;
    ...
    [self.totalBtn setImage:[UIImage imageNamed:@"checkall"] forState:UIControlStateNormal];
...
- (IBAction)totalBtnClicked:(UIButton *)sender {
    ...
    if (self.isCheckAll) {
        ...
        [self.totalBtn setImage:[UIImage imageNamed:@"checkall"] forState:UIControlStateNormal];
         ...
         [self.totalBtn setImage:[UIImage imageNamed:@"checkall_p"] forState:UIControlStateNormal];
     }
...
- (void)updateTipsLabel{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.isNoMoreFiles) {
            ...
            NSString *tips = [NSString stringWithFormat:NSLocalizedString(@"%d", nil),(int)self.dataSource.count];
...
- (void)didBeganLongPress:(APKDVRFileCell *)cell{
    
    self.tableView.editing = YES;
}

- (void)didEndedLongPress:(APKDVRFileCell *)cell{
